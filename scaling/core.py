...
class Chinchilla:
    ...
    def fit(self, parallel: bool = True, simulation: bool = False) -> None:
        ...
        def _optimize_params(i):
            ...
            # x, lbfgs_loss, info_dict = sciop.fmin_l_bfgs_b(
            ...
            #     self._evaluate_params,
            #     x0,
            #     args=(self._const["log_N"], self._const["log_D"], self._const["y_true"]),
            #     fprime=grad(self._evaluate_params),
            #     # maxiter=1_000_000,
            #     # maxfun=1_000_000,
            #     factr=10,
            #     # Default values generally perform fine
            # )
            # if np.isfinite(lbfgs_loss):
            #     return x, lbfgs_loss, info_dict
            

            # result = sciop.minimize(self._evaluate_params, x0, jac=grad(self._evaluate_params), args=(self._const["log_N"], self._const["log_D"], self._const["y_true"]), method='BFGS', tol=1e-1)
            
            # print(result)
            # if result.success:
            #     return result.x, result.fun, {'func_calls': result.nfev, 'grad_calls': result.njev, 'warnflag': result.status}
        
            result = sciop.fmin_bfgs(
                self._evaluate_params,
                x0,
                args=(self._const["log_N"], self._const["log_D"], self._const["y_true"]),
                fprime=grad(self._evaluate_params),
            ...
            if np.isfinite(result[1]):
                return result[0], result[1], {'func_calls': result[4], 'grad_calls': result[5], 'warnflag': result[6]}
        ...
        if isinstance(best_fit[2], dict):
            self.logger.info("Optimization info" + ", ".join(f"{k}: {v}" for k, v in best_fit[2].items()))
        else:
            self.logger.info(f"Optimization result: {', '.join(str(x) for x in best_fit[2:])}")
    ...
    def _evaluate_params(self, x, N, D, y_true) -> np.ndarray:
        """
        ...
        log_term_2nd = a - alpha * N
        log_term_3rd = b - beta * D
