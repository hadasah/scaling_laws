...
class Database:
    ...
    **Default columns**:
    ...
    def __init__(
        ...
        self.filepath = csv_file if csv_file else os.path.join(self.project_dir, "df.csv") if self.project_dir else None

        if df is not None:
            self.df = df
        elif self.filepath and (os.path.exists(self.filepath) and os.path.getsize(self.filepath) > 0):
