...
python $ROOT_DIR/open_lm/ds/scripts/data_download.py \
    --output-dir $ROOT_DIR/ds/data/raw \
    --dataset $HF_DATASET \
    --train-data-files $TRAIN_DATA_FILES \
    --valid-data-files $VALID_DATA_FILES \
    --version-name $VERSION_NAME

python $ROOT_DIR/open_lm/ds/scripts/make_chunks.py \
    --input-files $ROOT_DIR/ds/data/raw/$HF_DATASET/$VERSION_NAME/valid.jsonl \
    --output-dir $ROOT_DIR/ds/data/preprocessed/$HF_MODEL/$HF_DATASET \
...
python $ROOT_DIR/open_lm/ds/scripts/make_chunks.py \
    --input-files $ROOT_DIR/ds/data/raw/$HF_DATASET/$VERSION_NAME/train.jsonl \
...
python -m open_lm.utils.make_wds_manifest --data-dir $ROOT_DIR/ds/data/preprocessed/$HF_MODEL/$HF_DATASET/chunk-size_2048-v_train_${VERSION_NAME}/

bash $ROOT_DIR/ds/scripts/train.sh 
