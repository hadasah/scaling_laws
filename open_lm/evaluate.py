...
def evaluate(model, data, start_epoch, args, global_step, writer):
    """
    ...
    if args.train_num_samples is not None:
        ## this is incorrect because train_num_samples is not always = actual samples/epoch
        log_data["train_tokens"] = start_epoch * args.train_num_samples * args.seq_len
    log_data["train_tokens_fixed"] = (global_step) * args.global_batch_size * args.seq_len
...
def evaluate_loop(model, data_list, start_epoch, args, global_step, writer):
    log_data_list = []
    for i, data in enumerate(data_list):
        ...
        log_data_list.append(evaluate(model, data, start_epoch, args_copy, global_step, writer))
