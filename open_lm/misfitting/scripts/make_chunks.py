...
def process_files(file_list, buffer, enc, buffer_lock, CHUNK_SIZE):
    remaining_tokens = []
    queue = []
...
def consumer(my_id, output_dir, threads, buffer, buffer_lock, num_consumers, upload_to_s3=False, CHUNK_SIZE=2048):
    output_directory = f"{output_dir}/{my_id}"
...
def main(
    ...
    # while os.path.isdir(f"{output_dir}/tars-{CHUNK_SIZE - 1}-v{str(i)}"):
    #     i+=1
    output_dir = f"{output_dir}/chunk-size_{CHUNK_SIZE - 1}-v_{str(i)}" if not version_name else f"{output_dir}/chunk-size_{CHUNK_SIZE - 1}-v_{version_name}"
    if not overwrite and os.path.isdir(output_dir) and (len(os.listdir(output_dir)) != 0):
        print(f"{output_dir} is not empty, terminating")
        return
    ...
    if tokenizer == "EleutherAI/gpt-neox-20b":
        ...
        enc = AutoTokenizer.from_pretrained(tokenizer, use_fast=use_fast)
    ...
    for i in range(num_workers):
        ...
        t = threading.Thread(
            ...
            args=(input_files[start:end], buffer, tokenize, buffer_lock, CHUNK_SIZE),
        )
        t.start()
        threads.append(t)
