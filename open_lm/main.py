...
def main(args):
    ...
    if args.wandb and is_master(args):
        ...
        wandb_config.update({"resume": "allow"})
        wandb.init(
    ...
    if not requires_training:
        ...
        metrics = evaluate_loop(model, data["val_list"], start_epoch, args, global_step, writer)
    ...
    while not done_training:
        ...
        if "val_list" in data and (epoch % args.val_frequency == 0 or done_training):
            ...
            try:
                evaluation_metrics = evaluate_loop(model, data["val_list"], epoch, args, global_step, writer)
